...
def _minify(basedir, factors=[], resolutions=[]):
    ...
    for r in factors:
        ...
        for r in factors + resolutions:
            ...
            os.makedirs(imgdir)
            check_output('scp {}/* {}'.format(imgdir_orig, imgdir), shell=True)  # cp

            ext = imgs[0].split('.')[-1]
            args = ' '.join(['magick mogrify', '-resize', resizearg, '-format', 'png', '*.{}'.format(ext)])  # mogrify
            print(args)
            os.chdir(imgdir)
            ...
            if ext != 'png':
                check_output('del {}\*.{}'.format(imgdir, ext), shell=True)  # rm
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False, load_imgs=True):
...
if __name__ == "__main__":
    scene_path = ''  # path of the scene folder (containing "images" folder and it's corresponding "poses_bounds.npy") # [one to one]
    downscale_factor = 4
