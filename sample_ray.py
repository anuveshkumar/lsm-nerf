...
def dialate_img(img, kernel_size=20):
    import cv2
...
class RaySamplerSingleImage(object):
    def __init__(self, data, device, resize_factor=1, render_stride=1):
        ...
        self.device = device
        H, W, self.intrinsics, self.c2w_mat = parse_camera(self.camera)
    ...
    def get_rays_single_image(self, H, W, intrinsics, c2w):
        ...
        :param c2w: 4 by 4 camera to world extrinsic
        :return:
        """
        u, v = np.meshgrid(np.arange(W)[::self.render_stride], np.arange(H)[::self.render_stride])
        u = u.reshape(-1).astype(dtype=np.float32)  # + 0.5
